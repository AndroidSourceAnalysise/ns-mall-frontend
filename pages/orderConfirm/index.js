...
Page({
    ...
    onLoad: function(options) {
        ...
        util.getPersonInfo().then(function (d) {
            self.setData({ isRecommender: d.con_type == 1 });
        });
        ...
        Promise.all([this.getUseableIntegral(), this.getDeuctRuleIntegral(), this.getRebuyDiscount()]).then(function(vals) {
            self.getDeuctMoney({
                detail: {
                    value: self.data.integral
                }
            });
        });
        setTimeout(function () {
            self.updateRealPayMoney();
        }, 1000);
    },
    ...
    getFreight: function() {
        ...
        return new Promise(function (resolve, reject) {
            wx.request({
                ...
                success: function (res) {
                    self.setData({
                        'order.distributionMoney': res.data
                    });
                    resolve();
                }
            });
    ...
    getRebuyDiscount: function() {
        ...
        return new Promise(function (resolve, reject) {
            wx.request({
                ...
                method: 'POST',
                data: {
                    paramKey: 're_consume_rate '
                },
                success: function (res) {
                    self.setData({
                        rebuyDiscount: res.data
                    });
                    resolve();
                }
            });
    ...
    getUseableCouponList: function() {
        ...
        wx.request({
            ...
            success: function(res) {
                ...
                self.setData({
                    useableCouponList: list
                });
                maxCoupon = self.getMaxDiscountCoupon();
                self.setData({
                    'order.couponId': maxCoupon.id || '',
                    'order.availableCoupon': maxCoupon
                });
            }
        });
    },
    ...
    getDeuctMoney: function(evt) {
        ...
        this.setData({
            useIntegral: val,
            deductMoney: val * this.data.deuctRule
        });
        this.updateRealPayMoney();
    },
    ...
    reduceNum: function(evt) {
        ...
        if (num > 1) {
            ...
            this.setData(obj);
            this.setData({
                'order.availableCoupon': this.getMaxDiscountCoupon()
            });
            this.getFreight().then(function () {
                self.updateRealPayMoney();
            });
        }
    },
    ...
    radioChange: function(evt) {
        this.setData({
            recommenderType: evt.detail.value
        });
    },
    setRecommender: function(evt) {
        this.setData({
            recommenderNo: evt.detail.value
        });
    },
    getCustomerMessage: function(evt) {
        this.setData({
            'order.remark': evt.detail.value
        });
    },
    ...
    selectCoupon: function() {
        if (!this.data.useableCouponList.length) {
            return;
        }
    ...
    updateRealPayMoney: function () {
        ...
        if (d.isRecommender && d.rebuyDiscount) {
            //计算复购价
            money = util.accMul(money, util.accDiv(d.rebuyDiscount, 10));
        }
        if (!couponDiscountMoney) {
            money = money;
        } else {
            //减去优惠券金额
            money = util.accSub(money, couponDiscountMoney);
        };
        //减去积分抵扣金额
        money = util.accSub(money, d.deductMoney);
        //加上运费价格
        money = util.accAdd(money, d.order.distributionMoney);

        self.setData({ 'order.realPayment': money });
    }
})